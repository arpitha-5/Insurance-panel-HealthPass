...
function appendTimeline(currentTimelineStr, status, updatedBy, note) {
    try {
...
router.post('/:id/request-documents', auth_1.authenticateJWT, (0, auth_1.requireRoles)(['ADMIN', 'CLAIMS_STAFF']), async (req, res) => {
    try {
        if (!req.user)
        ...
        const updatedTimeline = appendTimeline(claim.timeline, updatedStatus, req.user.name, `Requested missing document: ${documentName}`);
...
router.put('/:id/status', auth_1.authenticateJWT, (0, auth_1.requireRoles)(['ADMIN', 'CLAIMS_STAFF']), async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: 'Unauthenticated.' });
        const { id } = req.params;
...
router.post('/:id/adjudicate', auth_1.authenticateJWT, (0, auth_1.requireRoles)(['ADMIN', 'CLAIMS_STAFF']), async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: 'Unauthenticated.' });
        const { id } = req.params;
...
router.post('/:id/settle', auth_1.authenticateJWT, (0, auth_1.requireRoles)(['ADMIN', 'CLAIMS_STAFF']), async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: 'Unauthenticated.' });
        const { id } = req.params;
        ...
        const updatedTimeline = appendTimeline(claim.timeline, updatedStatus, 'System Auto-Pay', `Direct deposit cleared. Reference: ${txnRef}`);
        const updatedClaim = await prisma.claim.update({
            where: { id },
            data: {
                status: updatedStatus,
...
router.post('/ocr/simulate', auth_1.authenticateJWT, async (req, res) => {
    try {
