    ...
    try {
        ...
        if (user.twoFactorEnabled) {
            return res.json({
                requires2FA: true,
                email: user.email,
                tempToken: jwt.sign({ id: user.id, email: user.email, name: user.name, role: user.role, isTemp: true }, JWT_SECRET, { expiresIn: '5m' })
            });
        }
        // Direct Login (No 2FA)
...
router.post('/toggle-2fa', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: 'Unauthenticated.' });
...
// 6. Terminate Session
router.post('/sessions/:id/terminate', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: 'Unauthenticated.' });
        const { id } = req.params;
...
router.get('/audit-logs', auth_1.authenticateJWT, async (req, res) => {
    try {
