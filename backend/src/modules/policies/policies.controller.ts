  ...
  try {
    ...
    if (search) {
      filter.OR = [
        ...
        { holderName: { contains: search as string } },
        { holderEmail: { contains: search as string } }
      ];
    }
...
router.post('/validate', authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.post('/:id/sync-reminders', authenticateJWT, requireRoles(['ADMIN', 'SUPPORT_STAFF']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthenticated.' });
    const { id } = req.params;
...
router.post('/:id/payment', authenticateJWT, requireRoles(['ADMIN', 'SUPPORT_STAFF']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthenticated.' });
    const { id } = req.params;
...
router.post('/:id/renew', authenticateJWT, requireRoles(['ADMIN', 'SUPPORT_STAFF']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthenticated.' });
    const { id } = req.params;
...
router.put('/:id/status', authenticateJWT, requireRoles(['ADMIN', 'SUPPORT_STAFF']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthenticated.' });
    const { id } = req.params;
